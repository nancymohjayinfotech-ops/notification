...
class _StudentNotificationsPageState extends State<StudentNotificationsPage> {
  ...
  Future<void> _fetchNotifications() async {
    ...
    try {
      final data = await StudentNotificationService.getNotifications();
      setState(() {
  ...
  Future<void> _toggleRead(_NotificationItem item) async {
    if (!item.isRead) {
      await StudentNotificationService.markAsRead(item.id);
      setState(() => item.isRead = true);
    }
  }

  Future<void> _deleteNotification(_NotificationItem item) async {
    await StudentNotificationService.deleteNotification(item.id);
    _fetchNotifications();
  }
